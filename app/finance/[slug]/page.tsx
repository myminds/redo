...
// Generate static params for all finance posts
export async function generateStaticParams() {
  const posts = getAllPosts().filter(post => post.category === "finance");
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    ...
    description: post.excerpt || "Expert financial advice, investment strategies, cryptocurrency insights, and personal finance tips.",
    keywords: post.tags || ["finance", "investment", "cryptocurrency", "personal-finance"],
    openGraph: {
      title: post.title,
      description: post.excerpt,
      type: "article",
      url: `https://Reagdindonstyf.com/finance/${post.slug}`,
      ...
      <div className="w-full px-4 sm:px-6 lg:px-8">
        <div className="flex flex-col lg:flex-row gap-4 lg:gap-8 py-4 lg:py-8">
          ...
          <main className="flex-1 min-w-0 max-w-none order-1 lg:order-2">
            ...
            <article className="rounded-2xl border border-gray-200 dark:border-gray-700 p-4 sm:p-6 lg:p-8 bg-white dark:bg-gray-800 shadow-lg">
              ...
              {post.image && (
                <div className="w-full h-64 sm:h-80 relative mb-8 overflow-hidden rounded-xl">
                  <Image
                    src={post.image}
                    alt={post.title}
                    fill
